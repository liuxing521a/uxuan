...
import com.uxuan.core.Pool.DataPool;
import com.uxuan.core.Syner.DataBaseSyner;
...
final class DataPoolImpl implements DataPool, DataBaseSyner {
	...
	@Override
	public void put(Base gameBase) {
		getCacheBean(gameBase.getId()).cache(gameBase);
	}

	@Override
	public <T extends Base> T get(String Id) {
		final CacheBean cached = getCacheBean(Id);
		
		Base gameObject = cached.get(Id);
		if (Objects.nonNull(gameObject)) {
			gameObject.setUpdateTime(TimeUtil.systemTime());
			return (T)gameObject;
		} 
		
		if (cached.module() instanceof BaseObject) {
			gameObject = ((BaseObject)cached.module()).autoInstance(Id);
			return (T)gameObject;
		}
	...
	@Override
	public <T extends Base> boolean isCached(Class<T> clazz, String Id) {
		return getCacheBean(clazz).contains(Id);
	}

	@Override
	public <T extends Base> T remove(String Id) {
		return (T) getCacheBean(Id).remove(Id);
	}

	@Override
	public <T extends Base> T remove(Class<T> clazz, String Id) {
		return (T) getCacheBean(clazz).remove(Id);
	}

	@Override
	public <T extends Base> T newInstance(Class<T> clazz, String Id) {
		return (T) newInstance(getCacheBean(clazz), Id);
	}

	@Override
	public <T extends Base> T newInstance(String Id) {
	...
	private Base newInstance(CacheBean cached, String Id) {
