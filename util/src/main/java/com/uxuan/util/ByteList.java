 ...
 */
public class ByteList implements Comparable<ByteList>, CharSequence, Serializable {
    ...
    public ByteList(ByteList wrap) {
        this(wrap.bytes, wrap.begin, wrap.realSize, true);
    }
...
//        assert newLength >= realSize : "newLength is too small";
   ...
   /**
     ...
     * @return a copy of the bytes.
     */
    public int bytes(byte[] bs) {
    	int realsize = Math.min(bs.length, realSize);
        System.arraycopy(bytes, begin, bs, 0, realsize);
        return realsize;
    }
